...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const onClick = (e: MouseEvent) => {
      if (!userMenuWrapRef.current?.contains(e.target as Node))
        setUserMenuOpen(false);
    };
  ...
  const openSubscriptionEntry = async () => {
    ...
    try {
      ...
      const subscription = pickCurrentSubscription(
        await readSubscriptionList(response),
      );
      if (!subscription) {
        router.push("/subscribe");
        return;
    ...
    <nav
      className={`fixed top-0 left-0 right-0 z-50 backdrop-blur-md border-b ${
        isTimeDeal
          ...
          : "bg-background-cream/90 border-background-200/70"
      }`}
    >
      <div className="w-full px-4 md:px-8 lg:px-12">
        <div className="flex items-center justify-between h-20 md:h-24">
          <Link href="/" className="flex items-center gap-2 shrink-0">
            <span
              className={`text-xl md:text-2xl font-semibold font-heading tracking-tight ${isTimeDeal ? "timedeal-nav-text text-white" : "text-foreground-950"}`}
            >
              Ounce
            </span>
          </Link>
              ...
              className={
                currentPath === "/timedeal"
                  ? `flex items-center gap-1.5 text-sm font-bold transition-colors whitespace-nowrap ${
                      isTimeDeal
                        ? "timedeal-nav-text text-white"
                        : "text-foreground-950"
                    }`
                  : `flex items-center gap-1.5 text-sm font-bold transition-colors whitespace-nowrap ${
                      isTimeDeal
                        ? "timedeal-nav-text text-white hover:text-white"
              ...
              <div className="relative" ref={userMenuWrapRef}>
                <button
                  ...
                  className={`group flex items-center gap-1.5 text-sm font-medium transition-colors cursor-pointer ${
                    isTimeDeal
                      ? "timedeal-nav-text text-white/85 hover:text-white"
                ...
                >
                  ...
                  <span
                    className={
                      isTimeDeal
                        ? "timedeal-nav-text text-white/60"
                        : "text-foreground-500"
                    }
                  >
                    님
                ...
                {userMenuOpen && (
                  <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-background-200 rounded-xl shadow-lg overflow-hidden z-50 py-1">
                    ...
                    >
                      <i className="ri-calendar-check-line text-foreground-400 text-base" />{" "}
                      {subscriptionNavLoading ? "확인 중..." : "내 구독"}
                    </button>
                    <Link
                      href="/account"
                      className="flex items-center gap-3 px-4 py-2.5 text-sm text-foreground-700 hover:bg-background-100 transition-colors"
                    >
                      <i className="ri-user-line text-foreground-400 text-base" />{" "}
                      내 계정
                    </Link>
                    <Link
                      href="/orders"
                      className="flex items-center gap-3 px-4 py-2.5 text-sm text-foreground-700 hover:bg-background-100 transition-colors"
                    >
                      <i className="ri-file-list-3-line text-foreground-400 text-base" />{" "}
                      주문 내역
                    </Link>
                    <Link
                      href="/support"
                      className="flex items-center gap-3 px-4 py-2.5 text-sm text-foreground-700 hover:bg-background-100 transition-colors"
                    >
                      <i className="ri-customer-service-2-line text-foreground-400 text-base" />{" "}
                      고객센터
                    </Link>
                    ...
                    {showAdmin && (
                      <Link
                        ...
                        className="flex items-center gap-3 px-4 py-2.5 text-sm text-foreground-700 hover:bg-background-100 transition-colors"
                      >
                        <i className="ri-shield-user-line text-foreground-400 text-base" />{" "}
                        문의 관리
            ...
            {showGuest && (
              <Link
                ...
                className={`text-sm font-medium transition-colors whitespace-nowrap tracking-wide ${
                  isTimeDeal
                    ? "timedeal-nav-text text-white/80 hover:text-white"
          ...
          >
            <i
              className={`ri-menu-line text-2xl ${isTimeDeal ? "text-white" : "text-foreground-950"}`}
            />
          </button>
        </div>
      </div>
        ...
        <div className="md:hidden bg-background-50/98 backdrop-blur-md border-t border-background-200 shadow-lg">
          <div className="px-4 py-5">
            ...
            <div className="grid grid-cols-2 gap-2 mb-5">
              ...
              >
                <i className="ri-grid-line text-primary-500 text-lg" /> 전체
                밀키트
              </Link>
              ...
              {CATEGORIES.map((c) => (
                <Link
                  ...
                  className="flex items-center gap-2.5 px-3 py-3.5 text-sm font-medium text-foreground-700 bg-background-100/60 hover:bg-background-100 rounded-xl transition-colors"
                >
                  <i className={`${c.icon} text-primary-500 text-lg`} />{" "}
                  {c.label}
                </Link>
              ))}
            </div>
            ...
            <div className="flex flex-col gap-0.5">
              {showGuest && (
                ...
                >
                  <i className="ri-login-box-line text-foreground-400 text-base" />{" "}
                  로그인
                </Link>
              )}
              ...
              {showAdmin && (
                ...
                >
                  <i className="ri-settings-4-line text-gray-500 text-base" />{" "}
                  관리자 대시보드
