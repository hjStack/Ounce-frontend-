...
export const CATEGORIES: Category[] = [
    { key: "stew", slug: "stew", label: "찌개·국물", icon: "ri-fire-line", desktop: true },
...
export function splitName(name?: string) {
    ...
    return {
        ...
        title: raw.replace(/^\s*\[[^\]]+\]\s*/, "").replace(/\s*밀키트\s*$/i, "").trim() || raw,
    };
}
...
export function categoryOf(product: Product) {
    const explicit = (product.category || product.categoryCode || "").toLowerCase();
    if (explicit && (explicit in CATEGORY_PATTERNS || explicit === "all")) return explicit;
...
export async function fetchCategoryCounts(keyword?: string) {
    ...
    const res = await fetch(`/api/products/category-counts${qs}`, { credentials: "include" });
