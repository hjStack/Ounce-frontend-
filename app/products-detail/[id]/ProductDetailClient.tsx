  ...
  const addToCart = async () => {
    ...
    try {
      const res = await apiFetch(
        `/api/carts/items?productId=${productId}&quantity=${quantity}`,
        {
          method: "POST",
          credentials: "include",
        },
        ...
        <div className="grid gap-8 md:grid-cols-[400px_1fr] md:gap-12">
          ...
          <div>
            ...
            <div className="mb-3 flex flex-wrap gap-1.5">
              {product.status === "TIME_DEAL" && (
                <Tag className="bg-deal-500 text-white">타임딜</Tag>
              )}
              {product.isBest && (
                <Tag className="bg-foreground-950 text-white">베스트</Tag>
              )}
              ...
              {product.unit && (
                <Tag className="bg-background-100 text-foreground-600">
                  {product.unit}
                </Tag>
              )}
            </div>

            <h1 className="text-2xl font-bold leading-snug text-foreground-950 md:text-[26px]">
              {product.name}
            </h1>
            ...
            {product.rating != null && (
              <div className="mt-2.5 flex items-center gap-2">
                ...
                <span className="text-sm text-foreground-500">
                  ({(product.reviewCount ?? 0).toLocaleString("ko-KR")}개 리뷰)
                </span>
              </div>
            )}
            ...
            <div className="mt-5">
              {isDiscounted(product) && (
                <div className="mb-1 flex items-center gap-2 text-sm">
                  ...
                  <span className="text-foreground-400 line-through">
                    {won(product.basePrice)}
                  </span>
                </div>
              )}
              {product.expirationDiscountPercent &&
                product.priceBeforeExpiration && (
                  <div className="mb-1 flex items-center gap-2 text-sm">
                    <span className="font-extrabold text-deal-500">
                      유통기한 임박 {product.expirationDiscountPercent}%
                      추가할인
                    </span>
                    <span className="text-foreground-400 line-through">
                      {won(product.priceBeforeExpiration)}
                    </span>
                  </div>
                )}
            ...
            <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <InfoCell
                label="배송"
                value="새벽배송 (오전 7시 전 도착) / 일반배송 (1~2일)"
              />
              {product.storage && (
                <InfoCell label="보관방법" value={product.storage} />
              )}
              {product.shelfLife && (
                <InfoCell label="유통기한" value={product.shelfLife} />
              )}
            </div>
            ...
            <div className="mt-5 flex gap-3 rounded-xl border border-background-200 bg-background-50 p-4 text-sm text-foreground-600">
              ...
              <div>
                <strong className="text-foreground-800">
                  {FREE_SHIPPING_THRESHOLD_LABEL} 이상 무료배송
                </strong>
                <p className="mt-0.5 text-foreground-500">
                  오늘 밤 11시 전 주문 시 다음 회차로 출고됩니다.
                </p>
              </div>
            </div>
          </div>
                ...
                <tbody>
                  {specs.map(([label, value]) => (
                    <tr key={label} className="border-b border-background-200">
                      ...
                      <td className="px-4 py-3.5 text-sm text-foreground-700">
                        {value}
                      </td>
                    </tr>
                  ))}
                  ...
                  <div className="mb-6 flex items-center gap-4 rounded-xl border border-background-200 bg-background-50 p-5">
                    ...
                    <div>
                      ...
                      <div className="mt-1 text-sm text-foreground-500">
                        {reviews.length.toLocaleString("ko-KR")}개 리뷰
                      </div>
                    </div>
                  </div>
                  <div>
                    {sortedReviews.map((review) => (
                      <div
                        key={review.reviewId}
                        className="border-b border-background-200 py-5"
                      >
          ...
          <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-background-200 px-5 py-4">
              ...
              <button
                ...
                aria-label="닫기"
                className="text-2xl text-foreground-500"
              >
                <i className="ri-close-line" />
              </button>
            </div>
            <div className="p-5">
              <div className="mb-2 text-sm font-semibold text-foreground-700">
                별점
              </div>
  ...
  return (
    <div className="flex gap-0.5">
      {[0, 1, 2, 3, 4].map((index) => (
        <i
          key={index}
          className={`${index < Math.round(rating) ? "ri-star-fill" : "ri-star-line"} text-sm text-yellow-400`}
        />
      ))}
    </div>
  );
  ...
  return (
    <button
      ...
      className={`relative flex-1 py-4 text-sm font-semibold transition-colors ${
        active
          ? "text-foreground-950 after:absolute after:inset-x-0 after:bottom-[-1px] after:h-0.5 after:bg-foreground-950"
          : "text-foreground-500"
      }`}
    >
      {children}
