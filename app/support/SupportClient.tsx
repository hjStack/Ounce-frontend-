...
const FAQS = [
  ...
  },
  {
    cat: "delivery",
    q: "언제 도착하나요?",
    a: "수도권은 주문 다음 날 새벽 7시 도착이 기준입니다. 지역에 따라 배송 방식과 도착 시간이 달라질 수 있습니다.",
  },
  {
    cat: "delivery",
  ...
  },
  {
    cat: "subscription",
    q: "구독하면 메뉴는 누가 정하나요?",
    a: "고객이 직접 고릅니다. 이번 주에 받을 밀키트를 내 구독 화면에서 하나씩 바꿀 수 있습니다.",
    href: "/subscription",
    link: "내 구독으로 이동",
  },
  {
    ...
    a: "내 구독 화면에서 한 주를 건너뛰거나 해지할 수 있도록 준비 중입니다.",
  },
  {
    cat: "refund",
    q: "주문을 취소하고 싶어요.",
    a: "주문 마감 전까지는 주문 내역에서 직접 취소할 수 있습니다. 마감 후에는 1:1 문의로 주문번호를 남겨주세요.",
    href: "/orders",
    link: "주문 내역 보기",
  },
  {
  ...
  },
  {
    cat: "account",
    q: "탈퇴하고 싶어요.",
    a: "내 계정 화면 아래쪽에서 직접 탈퇴할 수 있습니다. 탈퇴하면 주문 내역·포인트·쿠폰·장바구니가 모두 삭제됩니다.",
    href: "/account",
    link: "내 계정으로 이동",
  },
  {
...
export default function SupportClient() {
  ...
  const loadQnas = useCallback(
    async (page = 0) => {
      setQnaLoading(true);
      try {
        const res = await apiFetch(
          `/api/qna/me?page=${page}&size=${PAGE_SIZE}`,
          { credentials: "include" },
        );
        if (res.status === 401) {
          setQnas([]);
        ...
        setQnas((current) =>
          page === 0 ? nextItems : [...current, ...nextItems],
        );
        setQnaPage(page);
  ...
  const deleteQna = async (item: Qna) => {
    ...
    if (!ok) return;
    try {
      const res = await apiFetch(`/api/qna/${item.qnaId}`, {
        method: "DELETE",
        credentials: "include",
      });
      if (res.status === 401) {
      ...
      <main className="min-h-screen px-4 pb-20 pt-28 md:px-8">
        <div className="mx-auto w-full max-w-5xl">
          <div className="mb-7">
            ...
            <h1 className="text-2xl font-bold text-foreground-950 md:text-3xl">
              고객센터
            </h1>
            <p className="mt-1.5 text-sm text-foreground-500">
              자주 묻는 질문을 확인하고 로그인 후 나의 문의를 관리할 수
              있습니다.
            </p>
          </div>
            ...
            <section>
              <div className="mb-4 rounded-xl border border-background-200 bg-white p-4">
                ...
                <div className="flex gap-2 overflow-x-auto pb-1">
                  {[
                    { key: "all", label: "전체", icon: "ri-grid-line" },
                    ...QNA_CATEGORIES,
                  ].map((item) => (
                    <button
                      key={item.key}
                      ...
                      className={`flex shrink-0 items-center gap-1.5 rounded-full px-3.5 py-1.5 text-xs font-semibold transition-colors ${
                        faqCategory === item.key
                          ...
                          : "border border-background-200 bg-white text-foreground-600 hover:border-primary-300"
                      }`}
                    >
              ...
              <div className="flex flex-col gap-3">
                {filteredFaqs.map((faq, index) => (
                  ...
                  >
                    ...
                    >
                      <span className="mt-0.5 shrink-0 text-sm font-bold text-primary-500">
                        Q
                      </span>
                      <span className="flex-1 break-keep text-sm font-semibold text-foreground-900">
                        {faq.q}
                      </span>
                      <i
                        className={`ri-arrow-down-s-line shrink-0 text-lg text-foreground-400 transition-transform ${openFaq === index ? "rotate-180" : ""}`}
                      />
                    </button>
                    {openFaq === index && (
                      <div className="px-4 pb-4 pl-11">
                        <p className="break-keep text-sm leading-relaxed text-foreground-600">
                          {faq.a}
                        </p>
                        {faq.href && (
                          <Link
              ...
              {filteredFaqs.length === 0 && (
                <p className="py-16 text-center text-sm text-foreground-400">
                  해당하는 질문이 없습니다.
                </p>
              )}
            </section>
          ...
          {tab === "inquiry" && !authLoading && isAuthenticated && (
            <section className="grid gap-6 lg:grid-cols-[360px_1fr]">
              <form
                className="h-fit rounded-xl border border-background-200 bg-white p-5"
                onSubmit={submitQna}
              >
                <div className="mb-4 flex items-center justify-between">
                  <h2 className="text-sm font-bold text-foreground-950">
                    {editingId ? "문의 수정" : "문의 등록"}
                  </h2>
                  {editingId && (
                    <button
                      type="button"
                      onClick={resetForm}
                      className="text-xs font-medium text-foreground-400 hover:text-foreground-700"
                    >
                      취소
                    </button>
                  )}
                </div>
                <label className="mb-1.5 block text-xs text-foreground-500">
                  문의 유형
                </label>
                <select
                  value={category}
                ...
                <button
                  type="submit"
                  className="w-full rounded-lg bg-primary-500 px-5 py-3 text-sm font-bold text-white hover:bg-primary-600"
                >
                  {editingId ? "수정하기" : "등록하기"}
                </button>
              </form>
              ...
              <div className="flex flex-col gap-3">
                {qnaLoading && qnas.length === 0 && (
                  <div className="rounded-xl border border-background-200 bg-white py-12 text-center text-sm text-foreground-400">
                    문의 내역을 불러오는 중...
                  </div>
                )}
                {qnas.map((item) => (
                  ...
                  >
                    <button
                      type="button"
                      onClick={() =>
                        setOpenQna(openQna === item.qnaId ? null : item.qnaId)
                      }
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        <div className="mb-1.5 flex flex-wrap items-center gap-2">
                          ...
                          {item.category && (
                            <span className="rounded-full bg-background-100 px-2 py-0.5 text-[11px] text-foreground-500">
                              {item.category}
                            </span>
                          )}
                        ...
                        <p className="clamp-1 text-sm font-semibold text-foreground-900">
                          {item.title}
                        </p>
                      </div>
                      <i
                        className={`ri-arrow-down-s-line mt-1 shrink-0 text-lg text-foreground-400 transition-transform ${openQna === item.qnaId ? "rotate-180" : ""}`}
                      />
                    </button>
                    {openQna === item.qnaId && (
                      <div className="px-4 pb-4">
                        <p className="qna-body rounded-xl bg-background-100/70 px-4 py-3.5 text-sm leading-relaxed text-foreground-700">
                          {item.content}
                        </p>
                        {item.answer && (
                          <div className="mt-3 rounded-xl border border-primary-100 bg-primary-50/70 px-4 py-3.5">
                            <div className="mb-2 flex items-center gap-2">
                              ...
                              <span className="ml-auto text-[11px] text-primary-700/70">
                                {formatDate(item.answeredAt || undefined, true)}
                              </span>
                            </div>
                            <p className="qna-body text-sm leading-relaxed text-primary-900/90">
                              {item.answer}
                            </p>
                          </div>
                        )}
                        {item.status !== "ANSWERED" && (
                          <div className="mt-3 flex gap-2">
                            <button
                              ...
                              className="rounded-lg border border-background-200 px-3 py-2 text-xs font-semibold text-foreground-600 hover:bg-background-100"
                            >
                              수정
                            </button>
                            <button
                              type="button"
                              onClick={() => void deleteQna(item)}
                              className="rounded-lg border border-red-100 px-3 py-2 text-xs font-semibold text-red-500 hover:bg-red-50"
                            >
                              삭제
                            </button>
                          </div>
                ...
                {!qnaLoading && qnas.length === 0 && (
                  <div className="rounded-xl border border-background-200 bg-white py-16 text-center text-sm text-foreground-400">
                    등록된 문의가 없습니다.
                  </div>
                )}
                {!qnaLast && (
  ...
  return (
    <section className="rounded-xl border border-background-200 bg-white px-5 py-16 text-center">
      ...
      <p className="mt-2 text-sm text-foreground-500">
        나의 문의 내역과 답변은 계정에 연결되어 있습니다.
      </p>
      <div className="mt-6 flex flex-col justify-center gap-2 sm:flex-row">
        <Link
          href="/login"
          className="rounded-lg bg-primary-500 px-5 py-3 text-sm font-bold text-white hover:bg-primary-600"
        >
          로그인하기
        </Link>
        <Link
          href="/signup"
          className="rounded-lg border border-background-200 bg-white px-5 py-3 text-sm font-bold text-foreground-700 hover:bg-background-100"
        >
          회원가입
        </Link>
      </div>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-bold ${answered ? "bg-primary-50 text-primary-700" : "bg-deal-100 text-deal-700"}`}
    >
