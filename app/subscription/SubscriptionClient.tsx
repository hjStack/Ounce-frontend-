...
import { fetchCatalog, getProductPrice, PRODUCT_PLACEHOLDER, splitName, stockState, won } from "../../lib/products";
import {
    completeWeeklyMenu,
    normalizeWeeklyMenu,
    productIdsOf,
...
type ActionKey = "meals" | "menu" | "skip" | "pause" | "resume" | "cancelSkip" | "retry" | "checkout" | "cancel" | null;
...
function cycleDateOf(cycle: SubscriptionCycle) {
    return cycle.billingDate || cycle.billingAt || cycle.paidAt || cycle.paymentTriedAt || cycle.deliveryDate || cycle.deliveredAt || undefined;
}

function cycleStatusLabel(cycle: SubscriptionCycle) {
    const status = String(cycle.status || cycle.paymentStatus || cycle.deliveryStatus || "").toUpperCase();
...
function couponLabel(subscription: SubscriptionResponse) {
    if (subscription.freeShippingCouponIssued === true) return "3,000원 쿠폰 발급";
...
export default function SubscriptionClient() {
    ...
    const [pauseDate, setPauseDate] = useState(() => dateInputValue(addDays(new Date(), 7)));
    ...
    const pauseMinimumDate = dateInputValue(addDays(parseDate(nextPaymentDate) ?? new Date(), 1));
    ...
    const draftMenuAmount = draftMenu.reduce((sum, product) => sum + (product ? getProductPrice(product) : 0), 0);
    const menuComplete = draftProductIds.length === currentPlan.meals;
    const menuChanged = menuComplete && draftProductIds.join(",") !== savedProductIds.join(",");
    ...
    const mergeSubscription = useCallback((nextSubscription: SubscriptionResponse) => {
        ...
        setSubscriptions((items) => {
            ...
            return exists ? items.map((item) => (subscriptionIdOf(item) === id ? nextSubscription : item)) : [nextSubscription, ...items];
        });
    }, []);
    ...
    const loadSubscription = useCallback(async () => {
        ...
        try {
            ...
            if (!response.ok && response.status !== 404) throw new Error("SUBSCRIPTION_LOAD_FAILED");
    ...
    }, [catalog, catalogLoading, currentPlan.meals, nextDeliveryDate, nextPaymentDate, subscription, subscriptionId]);
    ...
    const applyDetailResponse = async (response: Response, fallback: string) => {
        ...
        const data = (await response.json().catch(() => null)) as SubscriptionResponse | null;
        if (data) {
    ...
    const selectMenuProduct = (product: Product) => {
        ...
        setDraftMenu((items) => {
            const next = Array.from({ length: currentPlan.meals }, (_, index) => items[index] ?? null);
    ...
    const saveMenu = async () => {
        ...
        if (!menuComplete) {
            toast(`메뉴 수량 합계가 주 ${currentPlan.meals}끼와 같아야 합니다.`, "error");
            return;
        }
    ...
    const checkoutNow = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                toast(await readApiMessage(response, "결제 요청에 실패했습니다."), "error");
                return;
            }
            ...
            toast(data?.message || (failed ? "결제에 실패했습니다." : "결제가 처리되었습니다."), failed ? "error" : "success");
        } catch {
            toast("서버와 통신 중 문제가 발생했습니다.", "error");
        } finally {
            setAction(null);
    ...
    const retryPayment = async () => {
        ...
        try {
            ...
            if (!response.ok && response.status !== 202) {
                toast(await readApiMessage(response, "결제 재시도 요청에 실패했습니다."), "error");
                return;
            }
    ...
    const skipThisWeek = async () => {
        ...
        try {
            ...
            if (data) {
                ...
                if (nextDate) {
                    setPauseDate(dateInputValue(addDays(parseDate(nextDate) ?? new Date(), 1)));
                }
    ...
    const resume = async () => {
        ...
        try {
            ...
            if (handleAuthFailure(response)) return;
            if (!response.ok) {
                toast(await readApiMessage(response, "쉬어가기 해제에 실패했습니다."), "error");
                return;
            }
    ...
    const cancelSubscription = async () => {
        ...
        try {
            ...
            if (!response.ok && response.status !== 204) {
                toast(await readApiMessage(response, "구독 해지에 실패했습니다."), "error");
                return;
            }
        ...
        return (
            <PageShell>
                <StatePanel
                    ...
                    description={subscriptions.length > 0 ? "해지된 구독 이력만 있습니다. 새 구독을 다시 시작할 수 있습니다." : "주 4-7끼와 첫 메뉴를 고르면 오늘 바로 구독 결제가 진행됩니다."}
                    action={
    ...
    return (
        <PageShell>
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                    <div>
                        <div className="mb-3 flex items-center gap-3">
                            ...
                            <span className="text-xs font-bold text-primary-500">My Subscription</span>
                        </div>
                        <h1 className="text-2xl font-bold tracking-tight text-foreground-950 md:text-3xl">내 구독</h1>
                        <p className="mt-1.5 text-sm text-foreground-500">메뉴와 끼니 수는 다음 결제일 23시 전까지 횟수 제한 없이 바꿀 수 있습니다.</p>
                    </div>
            ...
            <div className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-5">
                ...
                <SummaryCard label="다음 결제" value={formatDateLabel(nextPaymentDate, true)} />
                <SummaryCard label="다음 배송" value={formatDateLabel(nextDeliveryDate)} />
                <SummaryCard label="변경 마감" value={deadlineLabel} accent />
            </div>

            <div className="mb-6 grid gap-3 md:grid-cols-2">
                <NoticeCard icon="ri-time-line" title="23시 배송 마감" text="23시 전 결제 성공은 다음날 새벽배송, 23시 이후 결제 성공은 다다음 날 새벽배송으로 접수됩니다." />
                <NoticeCard icon="ri-coupon-3-line" title="4회 성공 쿠폰" text="구독 결제가 4회 연속 성공할 때마다 3,000원 할인 쿠폰을 드립니다." />
            </div>
            ...
            {paymentFailed && (
                <section className="mb-6 rounded-xl border border-red-100 bg-red-50 p-5">
                    <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        <div>
                            <p className="text-sm font-bold text-red-700">이번 회차 결제가 실패했습니다</p>
                            <p className="mt-1 text-sm text-red-600">
                                현재 재시도 {paymentRetryCountOf(subscription)}/3회 · 실패 주차 {failedWeeksOf(subscription)}/3주
                            </p>
                        </div>
                        <div className="flex flex-wrap gap-2">
                            <button type="button" onClick={() => void retryPayment()} disabled={action !== null} className="rounded-lg bg-red-600 px-4 py-2 text-xs font-bold text-white disabled:opacity-40">
                                {action === "retry" ? "요청 중..." : "결제 재시도"}
                            </button>
                            <button type="button" onClick={() => void checkoutNow()} disabled={action !== null || !menuComplete} className="rounded-lg border border-red-200 bg-white px-4 py-2 text-xs font-bold text-red-700 disabled:opacity-40">
            ...
            <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_380px]">
                <div className="min-w-0">
                    ...
                    <section className="mt-6 rounded-xl border border-background-200 bg-white p-5 shadow-sm md:p-6">
                        ...
                        <dl className="mt-6 grid gap-4 border-t border-background-200 pt-5 sm:grid-cols-2">
                            ...
                            <DetailLine label="상태" value={subscriptionStatusLabel(status)} strong />
                            <DetailLine label="구독 시작일" value={formatDateLabel(startedDate, true)} />
                            <DetailLine label="첫 결제일" value={formatDateLabel(startedDate, true)} />
                            <DetailLine label="다음 결제일" value={formatDateLabel(nextPaymentDate, true)} />
                            <DetailLine label="다음 배송일" value={formatDateLabel(nextDeliveryDate)} />
                            <DetailLine label="쉬어가기 종료" value={formatDateLabel(pausedUntil)} />
                            <DetailLine label="연속 성공 결제" value={`${maintainedWeeksOf(subscription).toLocaleString("ko-KR")}회`} />
                            <DetailLine label="쿠폰" value={couponLabel(subscription)} strong />
                            <DetailLine label="선택 메뉴 합계" value={draftMenuAmount > 0 ? won(draftMenuAmount) : "-"} />
                            <DetailLine label="해지일" value={formatDateLabel(canceledAtOf(subscription), true)} />
                        </dl>
                    </section>
                </div>
                ...
                <aside className="h-fit rounded-xl border border-background-200 bg-white p-5 shadow-sm">
                    <h2 className="text-lg font-bold text-foreground-950">구독 설정</h2>
                        <p className="mt-1 text-sm text-foreground-500">배송 1회를 건너뛰거나, 다시 시작할 날짜를 정할 수 있습니다.</p>
                    ...
                    <div className="mt-5 rounded-xl bg-background-100/70 p-4">
                        ...
                        <input
                            ...
                            value={draftMeals}
                            disabled={!canEdit}
                            onChange={(event) => setDraftMeals(clampMeals(event.target.value))}
                        ...
                        <button type="button" onClick={() => void changeMeals()} disabled={!canEdit || !mealsChanged || action !== null} className="mt-4 w-full rounded-lg bg-ink-500 px-4 py-3 text-sm font-semibold text-white hover:bg-ink-600 disabled:opacity-40">
                            {action === "meals" ? "변경 중..." : "끼니 수 변경"}
                        </button>
                    </div>

                    <div className="mt-5 grid gap-3">
                        <button type="button" onClick={() => void skipThisWeek()} disabled={!canEdit || !active || action !== null} className="flex w-full items-center justify-center gap-2 rounded-lg border border-background-200 bg-white px-4 py-3 text-sm font-semibold text-foreground-800 hover:border-primary-200 hover:text-primary-700 disabled:opacity-40">
                            <i className="ri-skip-forward-line" />
                            {action === "skip" ? "처리 중..." : "배송 1회 건너뛰기"}
                        </button>
                        <p className="-mt-1 px-1 text-xs leading-relaxed text-foreground-500">구독은 유지되며 다음 결제일에 자동으로 다시 시작됩니다.</p>

                        {active && (skipApplied || subscription?.skipCancelable === true) && (
                            <button type="button" onClick={() => void cancelSkip()} disabled={action !== null} className="-mt-1 flex w-full items-center justify-center gap-1.5 px-3 py-2 text-xs font-semibold text-primary-700 hover:text-primary-800 disabled:opacity-40">
                                <i className="ri-play-circle-line" />
                                {action === "cancelSkip" ? "해제 중..." : "배송 건너뛰기 취소"}
                            </button>
                        )}
                        <div className="rounded-xl bg-background-100/70 p-4">
                            <label htmlFor="subscription-pause-date" className="text-sm font-bold text-foreground-950">
                                결제·배송 일시중지
                            </label>
                            <p className="mt-1 text-xs leading-relaxed text-foreground-500">선택한 날짜 전까지 결제와 배송을 멈추고, 해당 날짜부터 다시 시작합니다.</p>
                            ...
                            />
                            <p className="mt-2 text-[11px] text-foreground-400">최소 선택 가능일 · {formatDateLabel(pauseMinimumDate)}</p>
                            <button type="button" onClick={() => void pauseUntil()} disabled={!canEdit || !active || !pauseDate || action !== null} className="mt-3 w-full rounded-lg bg-primary-500 px-4 py-3 text-sm font-semibold text-white hover:bg-primary-600 disabled:opacity-40">
                                {action === "pause" ? "설정 중..." : "이 날짜부터 다시 시작"}
                            </button>
                            {!active && (status === "PAUSED" || status === "SKIPPED") && (
                                <button type="button" onClick={() => void resume()} disabled={action !== null} className="mt-2 flex w-full items-center justify-center gap-1.5 px-3 py-2 text-xs font-semibold text-primary-700 hover:text-primary-800 disabled:opacity-40">
                                    <i className="ri-play-circle-line" />
                                    {action === "resume" ? "해제 중..." : "일시중지 취소하고 다음 결제일부터 다시 시작"}
                                </button>
                            )}
                        </div>
                    ...
                    <button type="button" onClick={() => setCancelConfirmOpen(true)} disabled={!current || action !== null} className="w-full rounded-lg border border-background-200 bg-white px-6 py-3 text-sm font-semibold text-red-500 transition-colors hover:border-red-200 hover:bg-red-50 disabled:opacity-40">
    ...
    return (
        <div className="bg-background-cream">
            <main className="min-h-screen pb-20 pt-20 md:pt-24">
                <div className="mx-auto w-full max-w-7xl px-4 py-6 md:px-8 md:py-8 lg:px-12">{children}</div>
            </main>
            <Footer />
        </div>
    );
...
function StatePanel({ icon, title, description, action }: { icon: string; title: string; description: string; action: ReactNode }) {
    return (
...
function SummaryCard({ label, value, accent = false }: { label: string; value: string; accent?: boolean }) {
    return (
        <div className="rounded-xl border border-background-200 bg-white p-4 shadow-sm">
            <p className="mb-1.5 text-[10px] font-semibold text-foreground-400">{label}</p>
            <p className={`break-keep text-base font-bold leading-tight md:text-lg ${accent ? "text-primary-600" : "text-foreground-950"}`}>{value}</p>
        </div>
    );
}
...
function SubscriptionDatePicker({
    ...
    const previousDisabled = previousMonth < new Date(minimum.getFullYear(), minimum.getMonth(), 1);
    ...
    return (
        <div className="relative mt-3">
            ...
            >
                <span className="flex items-center gap-2"><i className="ri-calendar-line text-base text-primary-600" />{formatDateLabel(value)}</span>
                <i className={`ri-arrow-down-s-line text-base text-foreground-400 transition-transform ${open ? "rotate-180" : ""}`} />
            </button>
            {open && (
                <div className="absolute left-0 right-0 top-[calc(100%+8px)] z-30 rounded-xl border border-background-200 bg-white p-3 shadow-xl" role="dialog" aria-label="일시중지 재개 날짜 선택">
                    <div className="flex items-center justify-between px-1 pb-3">
                        <button type="button" onClick={() => setMonth(previousMonth)} disabled={previousDisabled} className="flex h-8 w-8 items-center justify-center rounded-lg text-foreground-500 hover:bg-background-100 disabled:opacity-30" aria-label="이전 달"><i className="ri-arrow-left-s-line" /></button>
                        <strong className="text-sm text-foreground-950">{monthLabel}</strong>
                        <button type="button" onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() + 1, 1))} className="flex h-8 w-8 items-center justify-center rounded-lg text-foreground-500 hover:bg-background-100" aria-label="다음 달"><i className="ri-arrow-right-s-line" /></button>
                    </div>
                    ...
                    <div className="grid grid-cols-7 gap-1 text-center text-xs">
                        {cells.map((day, index) => {
                            ...
                            return <button key={dateValue} type="button" disabled={unavailable} onClick={() => { onChange(dateValue); setOpen(false); }} className={`h-8 rounded-lg font-medium transition-colors ${selectedDay ? "bg-primary-500 text-white" : unavailable ? "text-foreground-200" : "text-foreground-700 hover:bg-primary-50 hover:text-primary-700"}`}>{day}</button>;
                        })}
                    </div>
                    <p className="mt-3 border-t border-background-100 pt-2 text-[11px] text-foreground-400">{formatDateLabel(min)}부터 선택할 수 있습니다.</p>
                </div>
            )}
        </div>
    );
...
function NoticeCard({ icon, title, text }: { icon: string; title: string; text: string }) {
    return (
        <div className="flex min-h-24 items-start gap-3 rounded-xl border border-primary-100 bg-primary-50 p-4">
            ...
            <div>
                ...
                <p className="mt-1 break-keep text-xs leading-relaxed text-primary-700/80">{text}</p>
            </div>
        </div>
    );
}
...
function WeeklyMenuSection({
    ...
    const displaySlots = Array.from({ length: mealCount }, (_, index) => slots[index] ?? null);
    const canSave = active && menuComplete && (menuChanged || menuSource === "default");
        ...
        <section className="rounded-xl border border-background-200 bg-white shadow-sm">
            <div className="flex flex-col gap-4 border-b border-background-200 px-5 py-4 md:flex-row md:items-end md:justify-between md:px-6">
                <div>
                    <div className="mb-3 flex items-center gap-3">
                        ...
                        <span className="text-xs font-bold text-primary-500">Weekly Menu</span>
                    </div>
                    <h2 className="text-lg font-bold text-foreground-950">다음 회차 메뉴</h2>
                    <p className="mt-1 text-sm text-foreground-500">
                        {deadlineLabel}까지 변경 가능 · 남은 시간 {countdown}
                    </p>
                    <p className="mt-1 text-xs text-foreground-400">같은 메뉴도 여러 끼에 중복 선택할 수 있습니다.</p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                    <button type="button" onClick={onSave} disabled={!canSave || saving || catalogLoading} className="inline-flex items-center gap-1.5 rounded-lg bg-primary-500 px-4 py-2 text-xs font-bold text-white transition-colors hover:bg-primary-600 disabled:opacity-40">
            ...
            <div className="px-5 py-5 md:px-6">
                {catalogLoading ? (
                    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                        {Array.from({ length: mealCount }, (_, index) => (
                            <div key={index} className="h-28 animate-pulse rounded-lg bg-background-100" />
                        ))}
                    </div>
                ) : (
                    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                        {displaySlots.map((product, index) => (
                            <WeeklyMenuSlot
                                key={product ? `${product.productId}-${index}` : `empty-${index}`}
                ...
                {!menuComplete && !catalogLoading && (
                    <p className="mt-3 text-xs font-semibold text-red-500">메뉴 수량 합계가 주 {mealCount}끼와 같아야 저장할 수 있습니다.</p>
                )}
            </div>
        </section>
    );
...
function WeeklyMenuSlot({ product, index, disabled, onPick }: { product: Product | null; index: number; disabled: boolean; onPick: () => void }) {
    ...
    return (
        <article className="flex min-h-28 gap-3 rounded-lg bg-background-100 p-3">
            ...
            <div className="min-w-0 flex-1">
                <p className="text-xs font-bold text-primary-600">{DAY_LABELS[index % 7]}요일</p>
                <h3 className="mt-1 clamp-2 text-sm font-bold leading-snug text-foreground-950">{product ? parts.title : "메뉴 선택"}</h3>
                <p className="mt-1 text-xs text-foreground-500">{product ? parts.serving || product.unit || "1인분 밀키트" : "비어 있음"}</p>
            </div>
            <button type="button" onClick={onPick} disabled={disabled} className="self-start rounded-lg border border-background-200 bg-white px-3 py-2 text-xs font-bold text-foreground-700 transition-colors hover:border-primary-200 hover:text-primary-700 disabled:opacity-40">
                {product ? "변경" : "선택"}
            </button>
        </article>
    );
            ...
            <div className="flex max-h-[86vh] w-full max-w-3xl flex-col rounded-xl bg-white shadow-2xl">
                <div className="flex items-center justify-between border-b border-background-200 px-5 py-4">
                    <div>
                        ...
                        <p className="mt-1 text-xs text-foreground-500">같은 메뉴도 여러 끼에 중복 선택할 수 있습니다.</p>
                    </div>
                    <button type="button" onClick={onClose} className="rounded-lg p-2 text-foreground-500 hover:bg-background-100" aria-label="닫기">
                    ...
                    {loading ? (
                        <div className="grid gap-3 sm:grid-cols-2">
                            {Array.from({ length: 6 }, (_, index) => (
                                <div key={index} className="h-24 animate-pulse rounded-lg bg-background-100" />
                            ))}
                        </div>
                        ...
                        <div className="py-12 text-center text-sm text-foreground-400">선택할 수 있는 메뉴가 없습니다.</div>
                    )}
                </div>
            </div>
        </div>
...
function MenuProductOption({ current, product, onSelect }: { current: boolean; product: Product; onSelect: () => void }) {
    const parts = splitName(product.name);
    ...
    return (
        ...
        >
            ...
            <span className="min-w-0 flex-1">
                <span className="clamp-2 text-sm font-bold leading-snug text-foreground-950">{parts.title}</span>
                <span className="mt-1 block text-xs text-foreground-500">{parts.serving || product.unit || "1인분 밀키트"}</span>
                <span className={`mt-2 inline-flex rounded-full px-2 py-1 text-[11px] font-bold ${current ? "bg-primary-500 text-white" : soldOut ? "bg-red-50 text-red-500" : "bg-primary-50 text-primary-700"}`}>
...
function CancelConfirmModal({
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/45 px-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="cancel-subscription-title">
            <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-2xl">
                <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-full bg-red-50 text-red-600">
                    <i className="ri-error-warning-line text-2xl" />
                </div>
                <h2 id="cancel-subscription-title" className="text-xl font-bold text-foreground-950">정말 해지하시겠습니까?</h2>
                <p className="mt-2 break-keep text-sm leading-relaxed text-foreground-500">
                    구독을 해지해도 이미 결제된 회차는 취소되지 않고 예정대로 배송됩니다. 다음 회차부터 결제와 배송이 중단되며, 다시 이용하려면 새 구독을 시작해야 합니다.
                </p>
                <div className="mt-6 grid gap-2">
                    <button type="button" onClick={onConfirm} disabled={canceling} className="rounded-lg bg-red-600 px-4 py-3 text-sm font-bold text-white hover:bg-red-700 disabled:opacity-40">
                        {canceling ? "해지 중..." : "구독 해지하기"}
                    </button>
                    <button type="button" onClick={onClose} disabled={canceling} className="rounded-lg border border-background-200 bg-white px-4 py-3 text-sm font-semibold text-foreground-700 hover:bg-background-50 disabled:opacity-40">
                        계속 이용하기
                    </button>
                </div>
            </div>
...
function CycleSection({ cycles, draftAmount }: { cycles: SubscriptionCycle[]; draftAmount: number }) {
    return (
        <section className="mt-8 overflow-hidden rounded-xl border border-background-200 bg-white shadow-sm">
            <div className="border-b border-background-200 px-5 py-4">
                ...
                <p className="mt-1 text-sm text-foreground-500">결제, 배송, 쉬어가기 내역을 확인합니다.</p>
            </div>
            {cycles.length === 0 ? (
                <div className="py-12 text-center text-sm text-foreground-400">표시할 회차 이력이 없습니다.</div>
            ) : (
                    ...
                    <table className="w-full min-w-[760px] text-left text-sm">
                        ...
                        <tbody className="divide-y divide-background-200 text-foreground-700">
                            {cycles.map((cycle, index) => {
                                const amount = cycle.status === "DRAFT" && draftAmount > 0
                                    ? draftAmount
                                    : Number(cycle.amount ?? cycle.totalAmount ?? 0);
                                return (
                                    <tr key={cycleIdOf(cycle) || index}>
                                        <td className="px-5 py-3 font-bold text-foreground-950">{cycleIdOf(cycle) > 0 ? `#${cycleIdOf(cycle)}` : index + 1}</td>
                                        <td className="px-5 py-3">{cycleStatusLabel(cycle)}</td>
                                        <td className="px-5 py-3">{formatDateLabel(cycle.billingDate || cycle.billingAt || cycle.paidAt, true)}</td>
                                        <td className="px-5 py-3">{formatDateLabel(cycle.deliveryDate || cycle.deliveredAt)}</td>
                                        <td className="px-5 py-3">{Number(cycle.failedAttempts ?? cycle.retryCount ?? 0)}/3회</td>
                                        <td className="px-5 py-3">{amount > 0 ? won(amount) : "-"}</td>
                                    </tr>
                                );
                            })}
                        </tbody>
