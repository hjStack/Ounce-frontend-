...
export default function TimeDealClient() {
    ...
    const sectionTitle = timer.active ? "지금 열린 미드나이트 특가" : "오늘의 미스터리 라인업";
                    ...
                    <div className="mb-7 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                        <div>
                            <div className="mb-3 flex items-center gap-3">
                                ...
                                <span className="text-xs font-bold text-deal-400">Limited Lineup</span>
                            </div>
                            <h2 className="text-2xl font-black text-white md:text-3xl">{sectionTitle}</h2>
                            <p className="mt-2 max-w-xl break-keep text-sm leading-6 text-white/45">{sectionDescription}</p>
                        </div>
                        {timer.active ? (
                            <div className="flex items-center gap-2 overflow-x-auto pb-1">
                                <span className="mr-1 shrink-0 text-sm text-white/40">정렬</span>
                                {SORTS.map((item) => (
                                    <button
                                        key={item.key}
                                        type="button"
                                        ...
                                        className={`shrink-0 rounded-md px-4 py-2 text-sm font-bold transition-colors ${
                                            sort === item.key ? "bg-deal-500 text-white" : "bg-white/[0.07] text-white/50 hover:bg-white/[0.12]"
                                        }`}
                                    >
                                        {item.label}
                                    </button>
                            ...
                            <div className="inline-flex w-fit items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-2 text-xs font-bold text-white/45">
                                <i className="ri-lock-2-line text-deal-400" />
                                밤 10시 공개 대기
                            </div>
                        )}
                    </div>
                    ...
                    {loading && (
                        <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
                            {Array.from({ length: 8 }, (_, index) => (
                                <div key={index} className="h-80 animate-pulse rounded-lg border border-white/5 bg-white/5" />
                            ))}
                        </div>
                    )}
...
function TimeDealCard({ product }: { product: Product }) {
    ...
    return (
        <article className="group overflow-hidden rounded-lg border border-white/10 bg-[#111318] shadow-[0_18px_44px_rgba(0,0,0,0.22)] transition-all duration-300 hover:-translate-y-1 hover:border-deal-400/35">
            ...
            <div className="p-4">
                <h2 className="clamp-2 min-h-10 text-sm font-bold leading-5 text-white">{parts.title}</h2>
                ...
                >
                    {!soldOut && !adding && <i className="ri-shopping-bag-3-line text-base" />}
    ...
    return (
        <div className="flex flex-col items-center">
            <div className="flex h-16 w-16 items-center justify-center rounded-sm border border-white/10 bg-white/5">
                <span className={`text-2xl font-semibold tabular-nums ${accent ? "text-deal-400" : ""}`}>{value}</span>
            </div>
