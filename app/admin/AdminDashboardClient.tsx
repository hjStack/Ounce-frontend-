...
function emptyOrders(overrides: Partial<DashboardOrders> = {}): DashboardOrders {
    return { items: [], total: 0, missing: false, failed: false, forbidden: false, ...overrides };
}
...
function orderDateValue(order: Order) {
    return order.createdAt || order.createdDate || order.created_at || order.orderDate || order.orderedAt || order.ordered_at || order.paidAt || order.paid_at || undefined;
}

function paymentDateValue(order: Order) {
    return order.paidAt || order.paid_at || order.createdAt || order.createdDate || order.created_at || order.orderDate || order.orderedAt || order.ordered_at || undefined;
}
...
async function fetchDashboardOrders(): Promise<DashboardOrders> {
    try {
        ...
        return { ...emptyOrders(), items, total: readTotalOrders(data, items.length) };
    } catch {
...
export default function AdminDashboardClient() {
    ...
    const loadDashboard = useCallback(async () => {
        setLoading(true);
        const [orders, qna] = await Promise.all([fetchDashboardOrders(), fetchWaitingQna()]);
    ...
    const stats = useMemo(() => {
        const datedOrders = ordersData.items.filter((order) => Boolean(orderDateValue(order)));
        const todayOrders = datedOrders.filter((order) => isToday(orderDateValue(order)));
        const paidDatedOrders = ordersData.items.filter((order) => Boolean(paymentDateValue(order)));
        const todayPaidOrders = paidDatedOrders.filter((order) => isPaidOrder(order) && isToday(paymentDateValue(order)));
        const todayRevenue = todayPaidOrders.reduce((sum, order) => sum + orderAmount(order), 0);
        const paymentWaiting = ordersData.items.filter((order) => order.status === "PAYMENT_WAITING").length;
        const preparing = ordersData.items.filter((order) => order.status === "PREPARING").length;
        const shipped = ordersData.items.filter((order) => order.status === "SHIPPED").length;
        ...
        return [
            {
                ...
                color: "blue",
                note: datedOrders.length > 0 ? `결제 완료 ${todayPaidOrders.length.toLocaleString("ko-KR")}건 기준` : "주문일 응답 필드 필요",
            },
            {
                label: "신규 주문",
                value: datedOrders.length > 0 ? `${todayOrders.length.toLocaleString("ko-KR")}건` : "-",
            ...
            },
            {
                ...
                color: "red",
                note: qnaData.items.length > 0 ? `최근 ${qnaData.items.length.toLocaleString("ko-KR")}건 조회` : "답변 대기 문의",
            },
        ];
            ...
            {forbidden ? (
                ...
                <div className="space-y-6">
                    <section className="flex flex-col gap-4 rounded-xl border border-gray-100 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <p className="text-xs font-bold uppercase tracking-wide text-primary-600">Admin Overview</p>
                            <h2 className="mt-1 text-xl font-bold text-gray-900">운영 현황을 확인하세요</h2>
                            <p className="mt-1 text-sm text-gray-500">주문, 배송, 고객 문의 데이터를 실시간으로 조회합니다.</p>
                        </div>
                    ...
                    {missingApis.length > 0 && (
                        <div className="rounded-xl border border-yellow-100 bg-yellow-50 px-4 py-3 text-sm font-semibold text-yellow-800">
                            {missingApis.join(", ")}가 아직 연결되지 않아 해당 영역은 빈 값으로 표시됩니다.
                        </div>
                    )}
                    ...
                    <section className="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm">
                        <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4 md:px-6">
                            <div>
                                ...
                                <p className="mt-1 text-xs text-gray-400">관리자 주문 API의 최신 조회 결과 기준입니다.</p>
                            </div>
                        ...
                        {loading ? (
                            <div className="py-16 text-center text-sm text-gray-400">최근 주문을 불러오는 중...</div>
                        ) : recentOrders.length === 0 ? (
                            <div className="py-16 text-center">
                                <i className="ri-file-list-3-line mb-3 block text-4xl text-gray-300" />
                                <p className="text-sm font-semibold text-gray-700">표시할 주문이 없습니다.</p>
                            </div>
                        ) : (
                                    ...
                                    <tbody className="divide-y divide-gray-100 text-gray-700">
                                        {recentOrders.map((order) => (
                                            <tr key={order.orderId} className="hover:bg-gray-50/70">
                                                <td className="px-5 py-4 font-bold text-gray-900">{orderCode(order)}</td>
                                                <td className="px-5 py-4">{customerName(order)}</td>
                                                <td className="max-w-[260px] truncate px-5 py-4">{summaryProduct(order)}</td>
                                                <td className="px-5 py-4 font-bold text-gray-900">{won(orderAmount(order))}</td>
                                                <td className="px-5 py-4 text-gray-500">{formatDate(orderDateValue(order), true) || "-"}</td>
...
function StatCard({
    ...
    item: { label: string; value: string; icon: string; color: string; note: string };
    loading: boolean;
}) {
    ...
    return (
        <div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
            <div className="mb-4 flex items-center justify-between gap-3">
                ...
                <div className={`flex h-10 w-10 items-center justify-center rounded-lg ${colorClasses[item.color]}`}>
                    <i className={`${item.icon} text-xl`} />
                </div>
            </div>
            <p className="truncate text-2xl font-bold text-gray-900">{loading ? "..." : item.value}</p>
            <p className="mt-2 truncate text-xs text-gray-500">{loading ? "데이터 조회 중" : item.note}</p>
        </div>
    );
}
...
function StatusBadge({ status }: { status: string }) {
    return (
        <span className={`rounded-md px-2.5 py-1 text-xs font-bold ${STATUS_BADGES[status] || "bg-gray-100 text-gray-600"}`}>
