...
export default function AdminQnaClient() {
  ...
  const query = useCallback(
    (nextStatus: string, nextPage: number, size = PAGE_SIZE) => {
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      if (waitingRes.ok) {
        ...
        setOverdueCount(
          content.filter((item) => hoursSince(item.createdAt) > SLA_HOURS)
            .length,
        );
      }
      if (answeredRes.ok) {
  ...
  const loadItems = useCallback(
    async (nextStatus: string, nextPage: number) => {
      setLoading(true);
      try {
        const res = await apiFetch(query(nextStatus, nextPage), {
          credentials: "include",
        });
  ...
  const visible = useMemo(() => {
    ...
    return items.filter((item) =>
      `${item.title} ${item.content} ${item.memberName || ""} ${item.category || ""}`
        .toLowerCase()
        .includes(q),
    );
  }, [items, keyword]);
        ...
        <>
          ...
          <section className="rounded-xl border border-gray-100 bg-white shadow-sm">
            ...
            <div className="flex flex-col gap-3 p-4">
              {loading && (
                <div className="rounded-xl bg-gray-50 py-16 text-center text-sm text-gray-400">
                  문의 목록을 불러오는 중...
                </div>
              )}
              {!loading &&
                visible.map((item) => (
                  ...
                  >
                    <button
                      type="button"
                      onClick={() =>
                        setOpenId(openId === item.qnaId ? null : item.qnaId)
                      }
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="mb-1.5 flex flex-wrap items-center gap-2">
                          ...
                          <span className="text-[11px] font-medium text-gray-600">
                            {item.memberName || `회원 ${item.memberId}`}
                          </span>
                          <span className="text-[11px] text-gray-300">
                            #{item.qnaId}
                          </span>
                        </div>
                        <p className="clamp-1 text-sm font-semibold text-gray-900">
                          {item.title}
                        </p>
                      </div>
                      <i
                        className={`ri-arrow-down-s-line mt-1 shrink-0 text-lg text-gray-400 transition-transform ${openId === item.qnaId ? "rotate-180" : ""}`}
                      />
                    </button>

                    {openId === item.qnaId && (
                      <div className="px-4 pb-4">
                        <p className="qna-body rounded-xl bg-gray-50 px-4 py-3.5 text-sm leading-relaxed text-gray-700">
                          {item.content}
                        </p>
...
function StatCard({
  ...
  return (
    <div
      className={`rounded-xl border p-4 ${danger ? "border-red-200 bg-red-50" : "border-gray-100 bg-white"}`}
    >
      <p className="mb-1.5 text-xs text-gray-500">{label}</p>
      <p
        className={`text-xl font-bold leading-none tabular-nums ${danger ? "text-red-600" : "text-gray-950"}`}
      >
        {value}
      </p>
    </div>
...
function elapsedBadge(item: Qna) {
  ...
  const label =
    hours >= 24
      ? `${Math.floor(hours / 24)}일 경과`
      : `${Math.floor(hours)}시간 경과`;
  return (
