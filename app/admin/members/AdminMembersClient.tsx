...
function readMembers(data: MemberListResponse | AdminMember[]) {
    ...
    return data.content ?? data.members ?? data.items ?? data.data ?? data.results ?? [];
}
...
function memberRoles(member: AdminMember) {
    return [member.role, member.authority, ...(member.authorities ?? []).map(roleValue), ...(member.roles ?? []).map(roleValue)]
        .filter(Boolean)
...
function joinedAt(member: AdminMember) {
    return formatDate(member.createdAt || member.createdDate || member.created_at || member.joinedAt || undefined, true) || "-";
}
...
function spentAmount(member: AdminMember) {
    return Number(member.totalSpentAmount ?? member.totalPaymentAmount ?? member.totalSpent ?? 0);
}
...
export default function AdminMembersClient() {
    ...
    const loadMembers = useCallback(
        async (nextPage: number) => {
            ...
            try {
                ...
                const data = (await response.json()) as MemberListResponse | AdminMember[];
                ...
                setSelectedId((current) =>
                    nextMembers.some((member) => member.memberId === current) ? current : nextMembers[0]?.memberId ?? null,
                );
            } catch {
    ...
    const filteredMembers = useMemo(() => {
        ...
        return members.filter((member) => {
            ...
            const filterMatched =
                ...
                (filter === "ADMIN" && admin) ||
                (filter === "MEMBER" && !admin) ||
                (filter === "INACTIVE" && ["INACTIVE", "WITHDRAWN", "DELETED", "SUSPENDED"].includes(status));

            if (!filterMatched) return false;
            if (!q) return true;

            return [member.memberId, memberName(member), member.email, member.phone, member.grade, roleLabel(member), member.status]
    ...
    const selectedMember = useMemo(
        () => filteredMembers.find((member) => member.memberId === selectedId) ?? filteredMembers[0] ?? null,
        [filteredMembers, selectedId],
    );

    const stats = useMemo(() => {
        const admins = members.filter(isAdmin).length;
        const inactive = members.filter((member) => ["INACTIVE", "WITHDRAWN", "DELETED", "SUSPENDED"].includes(memberStatus(member).toUpperCase())).length;
        const points = members.reduce((sum, member) => sum + Number(member.point || 0), 0);
        return [
            { label: "전체 회원", value: `${totalElements.toLocaleString("ko-KR")}명`, icon: "ri-group-line" },
            { label: "관리자", value: `${admins.toLocaleString("ko-KR")}명`, icon: "ri-shield-user-line" },
            { label: "비활성", value: `${inactive.toLocaleString("ko-KR")}명`, icon: "ri-user-unfollow-line" },
            { label: "총 포인트", value: `${points.toLocaleString("ko-KR")}P`, icon: "ri-coin-line" },
        ];
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
                        <section className="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm">
                                        ...
                                        {FILTERS.map((item) => (
                                            <button
                                                ...
                                                className={`shrink-0 rounded-lg px-3.5 py-2 text-sm font-semibold transition-colors ${
                                                    filter === item.key ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-500 hover:text-gray-800"
                                                }`}
                                            >
                                                {item.label}
                                            </button>
                            ...
                            {loading ? (
                                <div className="py-16 text-center text-sm text-gray-400">회원 목록을 불러오는 중...</div>
                                            ...
                                            {filteredMembers.map((member) => {
                                                ...
                                                return (
                                                    <tr
                                                        ...
                                                        onClick={() => setSelectedId(member.memberId ?? null)}
                                                    ...
                                                    >
                                                        <td className="px-5 py-3">
                                                            <div className="flex min-w-0 items-center gap-3">
                                                                ...
                                                                <div className="min-w-0">
                                                                    <p className="truncate font-semibold text-gray-900">{memberName(member)}</p>
                                                        ...
                                                        <td className="px-5 py-3">
                                                            <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${isAdmin(member) ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-600"}`}>
                                                                {roleLabel(member)}
                                                            </span>
                                                        </td>
                                                        <td className="px-5 py-3 tabular-nums">{orderCount(member).toLocaleString("ko-KR")}건</td>
                                                        <td className="px-5 py-3 font-semibold text-gray-900">{won(spentAmount(member))}</td>
                                                        <td className="px-5 py-3 text-gray-500">{joinedAt(member)}</td>
                                                        <td className="px-5 py-3">
...
function StatusBadge({ status }: { status: string }) {
    ...
    const inactive = ["INACTIVE", "WITHDRAWN", "DELETED", "SUSPENDED"].includes(normalized);
    return (
        <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${inactive ? "bg-red-50 text-red-700" : "bg-green-50 text-green-700"}`}>
            {inactive ? "비활성" : "활성"}
        </span>
    );
}

function MemberDetail({ member, loading }: { member: AdminMember | null; loading: boolean }) {
    if (loading) {
        return <aside className="rounded-xl border border-gray-100 bg-white p-5 text-center text-sm text-gray-400 shadow-sm">회원 정보를 불러오는 중...</aside>;
    }

    if (!member) {
        return <aside className="rounded-xl border border-gray-100 bg-white p-5 text-center text-sm text-gray-400 shadow-sm">선택된 회원이 없습니다.</aside>;
    }
        ...
        <aside className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-4">
                <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                        <h2 className="truncate text-lg font-bold text-gray-950">{memberName(member)}</h2>
                        <p className="mt-0.5 truncate text-sm text-gray-500">{member.email || "-"}</p>
                    </div>
                </div>
                <StatusBadge status={memberStatus(member)} />
            </div>
            ...
            <dl className="mt-5 space-y-3 border-t border-gray-100 pt-5 text-sm">
                <DetailLine label="회원번호" value={member.memberId ? `#${member.memberId}` : "-"} />
                <DetailLine label="권한" value={roleLabel(member)} />
                <DetailLine label="등급" value={member.grade || "BASIC"} />
                <DetailLine label="포인트" value={`${Number(member.point || 0).toLocaleString("ko-KR")}P`} strong />
                <DetailLine label="주문 수" value={`${orderCount(member).toLocaleString("ko-KR")}건`} />
                <DetailLine label="누적 결제" value={won(spentAmount(member))} strong />
                <DetailLine label="보유 쿠폰" value={`${Number(member.couponCount || 0).toLocaleString("ko-KR")}장`} />
                <DetailLine label="사용 가능 쿠폰" value={`${Number(member.availableCouponCount || 0).toLocaleString("ko-KR")}장`} />
                <DetailLine label="연락처" value={member.phone || "-"} />
                <DetailLine label="가입일" value={joinedAt(member)} />
                <DetailLine label="최근 로그인" value={formatDate(member.lastLoginAt || member.lastLoginDate || undefined, true) || "-"} />
            ...
            {(member.address || member.defaultAddress) && (
                <div className="mt-5 rounded-lg bg-gray-50 px-4 py-3">
                    ...
                    <p className="mt-1 break-keep text-sm font-medium leading-6 text-gray-800">{member.address || member.defaultAddress}</p>
                </div>
            )}
        </aside>
    );
...
function EmptyState() {
    return (
        <div className="py-16 text-center">
            ...
            <p className="text-sm font-semibold text-gray-700">조건에 맞는 회원이 없습니다.</p>
