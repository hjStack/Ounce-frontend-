...
export default function ProductRegister() {
    ...
    if (authLoading) {
        return <div className="rounded-xl border border-gray-100 bg-white p-8 text-center text-sm text-gray-400">관리자 권한을 확인하는 중...</div>;
    }
    ...
    return (
        <div className="space-y-6">
            <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
                <form onSubmit={onSubmit} className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm md:p-6">
                    <div className="mb-6 flex flex-col gap-3 border-b border-gray-100 pb-5 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <p className="mt-1 text-sm text-gray-500">상품 정보와 이미지를 입력한 뒤 등록하세요.</p>
                        </div>
                        {createdId && (
            ...
            <section className="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm">
                <div className="flex flex-col gap-3 border-b border-gray-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-500">현재 조회 가능한 상품을 확인하고 가격, 할인율, 재고, 상태를 관리할 수 있습니다.</p>
                    </div>
                    <button
                        type="button"
                ...
                {productsLoading ? (
                    <div className="py-16 text-center text-sm text-gray-400">상품 목록을 불러오는 중...</div>
                ) : products.length === 0 ? (
                    <div className="py-16 text-center text-sm text-gray-400">조회 가능한 상품이 없습니다.</div>
                ) : (
                                        ...
                                        <td className="px-5 py-3">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <p className="mt-0.5 text-xs text-gray-400">#{product.productId}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-5 py-3 font-semibold text-gray-900">{won(product.basePrice)}</td>
                                        <td className="px-5 py-3">{Number(product.discountPercent || 0)}%</td>
                                        <td className="px-5 py-3 tabular-nums">{Number(product.stock || 0).toLocaleString("ko-KR")}개</td>
                                        <td className="px-5 py-3">
                                            <span className="rounded-full bg-gray-100 px-2.5 py-1 text-xs font-bold text-gray-600">{product.status}</span>
                                        </td>
                                        <td className="px-5 py-3">
