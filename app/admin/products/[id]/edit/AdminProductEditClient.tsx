...
export default function AdminProductEditClient({ productId }: { productId: string }) {
        ...
        async function loadProduct() {
            ...
            try {
                ...
                setForm({
                    ...
                    categoryIds: Array.isArray(data.categoryIds) ? data.categoryIds.join(",") : "",
                    description: data.description || "",
                });
            } catch (error) {
                if (!ignore) {
                    setProduct(null);
                    toast(error instanceof Error ? error.message : "상품 정보를 불러오지 못했습니다.", "error");
                }
            } finally {
    ...
    const previewImageUrl = imagePreviewUrl || product?.imageUrl || PRODUCT_PLACEHOLDER;

    const updateField = (field: keyof typeof INITIAL_FORM) => (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (authLoading || loading) {
        return <div className="rounded-xl border border-gray-100 bg-white p-8 text-center text-sm text-gray-400">상품 정보를 불러오는 중...</div>;
    }
    ...
    if (!product) {
        return (
            <div className="rounded-xl border border-gray-100 bg-white p-8 text-center">
                ...
                <p className="text-sm font-semibold text-gray-800">상품을 찾을 수 없습니다.</p>
                <Link href="/admin/products" className="mt-4 inline-flex rounded-lg bg-primary-500 px-4 py-2 text-sm font-bold text-white">
                    상품 관리로 이동
                </Link>
            </div>
        );
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <p className="text-xs font-bold uppercase tracking-wide text-primary-600">Product #{product.productId}</p>
                    <h2 className="mt-1 text-xl font-bold text-gray-900">상품 정보 수정</h2>
                </div>
                <div className="flex flex-wrap gap-2">
                    <Link href={`/products-detail/${product.productId}`} className="inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-600 hover:bg-gray-50">
                        <i className="ri-external-link-line" />
                        상품 보기
                    </Link>
                    <Link href="/admin/products" className="inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-600 hover:bg-gray-50">
            ...
            <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
                <form onSubmit={saveProduct} className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm md:p-6">
                    ...
                    <div className="mt-5">
                        <Field label="카테고리">
                            {categoryLoading ? (
                                <div className="rounded-lg border border-gray-200 bg-gray-50 px-3.5 py-3 text-sm text-gray-400">카테고리를 불러오는 중...</div>
                            ) : categories.length > 0 ? (
                                    ...
                                    {categories.map((category) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <span className="text-xs opacity-70">#{category.id}</span>
                                            </button>
                                        );
                                    })}
                                </div>
                            ) : (
                                <div className="rounded-lg border border-gray-200 bg-gray-50 px-3.5 py-3 text-sm text-gray-400">카테고리를 불러오지 못했습니다.</div>
                            )}
                        </Field>
                    </div>
                    ...
                    <div className="mt-6 flex flex-wrap gap-2">
                        ...
                        >
                            <i className={saving ? "ri-loader-4-line text-base" : "ri-save-3-line text-base"} />
                            {saving ? "저장 중..." : imageProcessing ? "이미지 처리 중..." : "변경사항 저장"}
                        </button>
                        <Link href="/admin/products" className="rounded-lg border border-gray-200 px-5 py-3 text-sm font-semibold text-gray-600 transition-colors hover:bg-gray-50">
                            취소
                        </Link>
                    </div>
                </form>
                ...
                <aside className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
                    ...
                    <div className="mt-5 space-y-2 rounded-lg bg-gray-50 p-4 text-xs leading-5 text-gray-500">
                        <div className="flex items-center justify-between">
                            <span>현재 가격</span>
                            <strong className="text-gray-800">{won(product.basePrice)}</strong>
                        </div>
                        <div className="flex items-center justify-between">
                            <span>현재 재고</span>
                            <strong className="text-gray-800">{Number(product.stock || 0).toLocaleString("ko-KR")}개</strong>
                        </div>
                        <div className="flex items-center justify-between">
                            <span>현재 상태</span>
...
async function requestProductUpdate(productId: string, body: FormData) {
    const endpoints = [`/api/admin/products/${productId}`, `/api/products/${productId}`];
...
function productUpdateErrorMessage(status: number, message: string) {
    if ([404, 405, 501].includes(status)) return "백엔드 상품 수정 API가 아직 연결되지 않았습니다.";
