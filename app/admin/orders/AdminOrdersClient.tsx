...
export default function AdminOrdersClient() {
  ...
  const loadOrders = useCallback(
    async (nextStatus: string, nextPage: number) => {
      setLoading(true);
      try {
        const response = await apiFetch(endpoint(nextStatus, nextPage), {
          credentials: "include",
        });
  ...
  const stats = useMemo(() => {
    const paidTotal = orders
      .filter(
        (order) => !["PAYMENT_WAITING", "CANCELED"].includes(order.status),
      )
    ...
    return [
      {
        label: "조회 주문",
        value: `${totalElements.toLocaleString("ko-KR")}건`,
        icon: "ri-file-list-3-line",
      },
      {
        label: "결제 대기",
        value: `${waiting.toLocaleString("ko-KR")}건`,
        icon: "ri-time-line",
      },
      {
        label: "출고 대상",
        value: `${(preparing + shipping).toLocaleString("ko-KR")}건`,
        icon: "ri-truck-line",
      },
                        ...
                        <Fragment key={order.orderId}>
                          <tr className="hover:bg-gray-50/70">
                            <td className="px-5 py-3">
                              ...
                              >
                                <i
                                  className={`ri-arrow-down-s-line text-lg text-gray-400 transition-transform ${open ? "rotate-180" : ""}`}
                                />
                                {orderCode(order)}
                              </button>
                            </td>
                            ...
                            <td className="max-w-[260px] px-5 py-3">
                              ...
                              <p className="mt-0.5 text-xs text-gray-400">
                                {orderItems(order).length.toLocaleString(
                                  "ko-KR",
                                )}
                                개 품목
                              </p>
...
function StatusBadge({ status }: { status: string }) {
  return (
    <span
      className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-bold ${STATUS_BADGES[status] || "bg-gray-100 text-gray-600"}`}
    >
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <div className="mt-3 divide-y divide-gray-100">
          {items.length > 0 ? (
            items.map((item, index) => (
              <div
                key={item.orderItemId ?? `${item.productId ?? "item"}-${index}`}
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-4">
        ...
        <dl className="mt-3 space-y-2 text-sm">
          <DetailLine
            label="수령인"
            value={order.receiverName || customerName(order)}
          />
          <DetailLine label="연락처" value={order.receiverPhone || "-"} />
          <DetailLine
            label="주소"
            value={order.shippingAddress || order.address || "-"}
          />
          <DetailLine
            label="배송비"
            value={order.shippingFee == null ? "-" : won(order.shippingFee)}
          />
          <DetailLine
            label="할인"
            value={
              order.discountAmount == null ? "-" : won(order.discountAmount)
            }
          />
...
function orderUpdateErrorMessage(status: number, message: string) {
  ...
  if ([404, 405, 501].includes(status))
    return "백엔드 주문 상태 변경 API가 아직 연결되지 않았습니다.";
