...
export default function CartClient() {
    ...
    const updateQuantity = async (cartId: number, delta: number) => {
        ...
        try {
            ...
            setItems((current) => current.map((cartItem) => (cartItem.cartId === cartId ? { ...cartItem, quantity: targetQty } : cartItem)));
            await refresh();
        } catch {
    ...
    const removeItems = async (targets: SelectableCartItem[]) => {
        ...
        const ok = await confirm(
            targets.length === 1 ? "선택하신 상품을 장바구니에서 뺄까요?" : `선택하신 ${targets.length}개의 상품을 삭제할까요?`,
            { kind: "delete", description: "삭제한 상품은 다시 상품 목록에서 담을 수 있습니다." },
        );
        if (!ok) return;
        ...
        try {
            ...
            setItems((current) => current.filter((item) => !removed.has(item.cartId)));
            await refresh();
            toast("상품이 삭제되었습니다.");
        } catch {
            toast("서버와의 통신에 실패했습니다.", "error");
    ...
    const handleCheckout = () => {
        ...
        sessionStorage.setItem("ounce.checkout.selectedCartIds", JSON.stringify(summary.selectedItems.map((item) => item.cartId)));
            ...
            <div className="bg-[#fcfbf9]">
                <main className="flex min-h-screen flex-col items-center justify-center px-4 py-32 pt-28">
                    <div className="max-w-sm text-center">
                        ...
                        <h1 className="mb-2 text-xl font-bold text-gray-900">장바구니가 비어 있어요</h1>
        ...
        <div className="bg-[#fcfbf9]">
            <main className="min-h-screen pb-20 pt-20 md:pt-24">
                <div className="mx-auto w-full max-w-7xl px-4 py-6 md:px-8 lg:px-12">
                    <div className="mb-6">
                        <h1 className="text-xl font-bold text-gray-900 md:text-2xl">장바구니</h1>
                        <p className="mt-1 text-sm text-gray-500">{summary.totalItems}개의 상품이 담겨 있어요</p>
                    </div>

                    <div className="mb-6 flex flex-col gap-2">
                        <div
                            className={`flex w-fit items-center gap-2 rounded-lg border px-3 py-2 ${
                                urgent ? "border-[#447861]/20 bg-[#447861]/10 text-[#447861]" : "border-[#3b4055]/15 bg-[#3b4055]/10"
                            }`}
                        >
                            <i className={`ri-time-line ${urgent ? "text-[#447861]" : "text-[#3b4055]"}`} />
                            <span className="text-xs font-medium text-[#3b4055]">
                                {urgent ? (
                                    <>
                                        오늘의 새벽배송 마감 임박! <strong className="font-bold text-[#396652]">{countdown}</strong> 후 마감
                                    </>
                                ) : (
                                    <>
                                        {pastCutoff ? "내일 밤 11시" : "오늘 밤 11시"}까지 주문 시{" "}
                                        <strong className="font-bold text-gray-900">{countdown}</strong> 남음 —{" "}
                                        {pastCutoff ? "모레 아침 7시" : "내일 아침 7시"} 전 도착
                                    </>
                                )}
                            </span>
                        </div>
                    ...
                    <div className="flex flex-col gap-8 lg:flex-row">
                        <div className="flex flex-1 flex-col gap-4">
                            <div className="flex items-center justify-between rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                                <label className="flex cursor-pointer items-center gap-2">
                                    <input
                                        type="checkbox"
                                        checked={summary.selectedCount === items.length && items.length > 0}
                                        onChange={(event) => setItems((current) => current.map((item) => ({ ...item, selected: event.target.checked })))}
                                ...
                                <button
                                    type="button"
                                    onClick={() => void removeItems(items.filter((item) => item.selected))}
                                          ...
                                          onToggle={(checked) =>
                                              setItems((current) =>
                                                  current.map((cartItem) =>
                                                      cartItem.cartId === item.cartId ? { ...cartItem, selected: checked } : cartItem,
                                                  ),
                                              )
                                          }
                                          onQuantity={(delta) => void updateQuantity(item.cartId, delta)}
                        ...
                        <div className="w-full shrink-0 lg:w-80">
                            <div className="sticky top-24 rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                                <h2 className="mb-4 text-sm font-bold text-gray-900">주문 요약</h2>
...
function CartRow({
    ...
    return (
        <div className="flex gap-4 rounded-lg border border-gray-200 bg-white p-4 shadow-sm transition-colors hover:border-gray-300">
            ...
            />
            <Link href={`/products-detail/${item.productId}`} className="h-20 w-20 shrink-0 overflow-hidden rounded-md border border-gray-100 bg-gray-50 md:h-24 md:w-24">
            ...
            <div className="flex min-w-0 flex-1 flex-col justify-between">
                <Link href={`/products-detail/${item.productId}`} className="clamp-2 text-sm font-medium text-gray-900 transition-colors hover:text-[#3b4055]">
                    {item.name}
                </Link>
                <div className="mt-2 flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button type="button" onClick={onRemove} className="flex h-7 w-7 items-center justify-center text-gray-400 transition-colors hover:text-red-500">
...
function SummaryRow({ label, value, valueClassName = "font-medium text-gray-800" }: { label: string; value: string; valueClassName?: string }) {
    return (
